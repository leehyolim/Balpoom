				...
				<div class="product_info">
					...
					<table class="product_table">
						...
						<%
							...
							int[] p_price = new int[Integer.parseInt(request.getParameter("index"))];
							double[] Amount_accumulated = new double[Integer.parseInt(request.getParameter("index"))];
						%>
						<%
							for (int i = 0; i < Integer.parseInt(request.getParameter("index")); i++) {
								...
								p_price[i] = Integer.parseInt(request.getParameter("price" + i));
								Amount_accumulated[i] = p_price[i]*0.01;
								
								System.out.println("결과값 : " + p_identifier[i]);
							}
						%>
						...
						<%for(int i = 0; i < Integer.parseInt(request.getParameter("index")); i++){ %>
							<tr>
								...
								<td>
									<div class="product_name" style="width: 60%; float: left;"><%=p_identifier[i] %></div>
								</td>
								<td class="product_amount"><%= c_cnt[i] %></td>
								<td class="product_price"><%= p_price[i] %></td>
								<!-- <td class="discount">-</td>
								<td class="discount_price">10000원</td> -->
								<%-- <fmt:parseNumber var="Amount_accumulated" value="<%=Amount_accumulated[i] %>" integerOnly="true"/>
								<td>${Amount_accumulated}</td> --%>
								<td><%= Math.round(Amount_accumulated[i])  %></td>
							</tr>
						<%} %>
						<%
							int totalprice = 0;
							for(int i = 0; i < Integer.parseInt(request.getParameter("index")); i++){
								totalprice = totalprice + p_price[i];
							}
						%>
						<tr>
							<%
						...
						<% 
							int rs_price = 0;
							for(int i = 0; i < Integer.parseInt(request.getParameter("index")); i++){
								rs_price = rs_price + p_price[i];
							}
							rs_price = rs_price - sel_price + delivery_fee;
						%>
